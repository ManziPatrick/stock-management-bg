...
class SaleServices extends BaseServices<any> {
  ...
  private async calculateStatistics(userId: string) {
    ...
    try {
      ...
      const dailySales = await SaleTransaction.find({
      ...
      const monthlySales = await SaleTransaction.find({
        user: new Types.ObjectId(userId),
        date: {
          $gte: startOfMonth,
          $lt: endOfMonth
      ...
      if (creditSaleIds.length > 0) {
        debitRecords = await DebitModel.find({
      ...
      for (const sale of monthlySales) {
        ...
        if (!monthlyProcessedTransactions.has(transactionKey)) {
          ...
          if (sale.status === 'credit') {
            ...
            monthlyTotalSales += relatedDebit?.paidAmount || sale.paidAmount || 0;
          } else {
            // For approved sales, use totalAmount
  ...
  private async calculateStatisticsWithAggregation(userId: string) {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
  ...
  async readAll(query: Record<string, unknown> = {}) {
    ...
    try {
      ...
      const data = await this.model.find(matchStage.$match).sort({ createdAt: -1 }).skip(skip).limit(limit).lean();
  ...
  private async addExpensesToDailyStats(dailyStats: any[], userId: string) {
    ...
    return dailyStats.map((stat) => {
      const matchingExpense = dailyExpenses.find(
        (exp) => exp._id.year === stat._id.year && exp._id.month === stat._id.month && exp._id.day === stat._id.day
      );
  ...
  private async addExpensesToMonthlyStats(monthlyStats: any[], userId: string) {
    ...
    return monthlyStats.map((stat) => {
      const matchingExpense = monthlyExpenses.find(
        (exp) => exp._id.year === stat._id.year && exp._id.month === stat._id.month
      );
    ...
    try {
      ...
      const previousMonthCredits = debitRecords.filter((debit) => {
        ...
        const isPreviousMonth =
          (debitDate.getMonth() === currentMonth - 1 && debitDate.getFullYear() === currentYear) ||
  ...
  async readAllDaily(query: { startDate?: string; endDate?: string }) {
    ...
    try {
      ...
      const sales = await this.model.find(matchStage.$match).lean();
      ...
      const finalStats = dailyStatsWithExpenses.map((stat) => {
        const matchingCredit = creditStats.find(
          (cs) => cs._id.year === stat._id.year && cs._id.month === stat._id.month && cs._id.day === stat._id.day
        );

        const matchingQuantity = quantityStats.find(
          (qs) => qs._id.year === stat._id.year && qs._id.month === stat._id.month && qs._id.day === stat._id.day
        );
  ...
  async readAllMonthly(query: { year?: string }) {
    ...
    try {
      ...
      const finalStats = monthlyStatsWithExpenses.map((stat) => {
        const matchingCredit = creditStats.find(
          (cs) => cs._id.year === stat._id.year && cs._id.month === stat._id.month
        );

        const matchingQuantity = quantityStats.find(
          (qs) => qs._id.year === stat._id.year && qs._id.month === stat._id.month
        );
  ...
  async updateDeliveryStatus(id: string, deliveryStatus: 'pending' | 'out_for_delivery' | 'delivered' | 'returned') {
    try {
      const updatedSale = await this.model.findByIdAndUpdate(
        id,
      ...
      throw new CustomError(400, error.message || 'Failed to update delivery status');
    }
  }
