...
class SellerControllers {
   ...
   */
  public readAll = asyncHandler(async (req, res) => {
    const result = await this.services.readAll(req.query); // Remove `req.user._id`
    ...
    sendResponse(res, {
      ...
      message: 'All records retrieved successfully', // Adjust the message to fit the context
      meta: {
        page,
        limit,
